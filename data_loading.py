...
DEFAULT_JOBSTREET_CSV = "jobstreet_all_jobs.csv"
DEFAULT_APPLICANTS_CSV = "job_applicants.csv"
...
def load_jobstreet_job_dataset(path: str = DEFAULT_JOBSTREET_CSV) -> pd.DataFrame:
    """
...
def load_applicants_dataset(path: str = DEFAULT_APPLICANTS_CSV) -> pd.DataFrame:
    """
...
def build_synthetic_interactions(
    ...
    for _, user in users.iterrows():
        ...
        if pref_loc:
            in_loc = candidates[candidates["job_location_lower"].str.contains(pref_loc,
        case=False,   # ignore upper/lowercase
        regex=False,  # <--- key fix: treat pref_loc as plain text, NOT regex
        na=False,)]
