...
LOCAL_JOBSTREET_CSV = DATA_DIR / "jobstreet_all_jobs.csv"
LOCAL_APPLICANTS_CSV = DATA_DIR / "job_applicants.csv"
...
def load_jobstreet_job_dataset():
    ...
    df = pd.read_csv(str(LOCAL_JOBSTREET_CSV))
...
def build_synthetic_interactions(users: pd.DataFrame, jobs: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, user in users.iterrows():
        ...
        if pref_loc:
            in_loc = cand[cand["job_location_lower"].str.contains(pref_loc, case=False, regex=False, na=False)]
